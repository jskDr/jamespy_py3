...
def open_multilayer_perceptron_dropout(_X, _keep_prob, _weights, _biases):
	...
	pred = tf.matmul(layer_2_dropout, _weights['out']) + _biases['out']
	
	return layer_1_dropout, layer_2_dropout, pred
...
def open_multilayer_perceptron(_X, _weights, _biases):
	"""
	In addition to out, hidden layer 1 and layer 2 will be fed back
	"""
	#Hidden layer with RELU activation
	layer_1 = tf.nn.relu(tf.add(tf.matmul(_X, _weights['h1']), _biases['b1'])) 
	#Hidden layer with RELU activation
...
class _DNN_r0():
	...
	Advanced version is built below.
	"""
...
class DNN():
	...
	def tf_machine_wb( self):
		x, y, n_input, n_hidden_1, n_hidden_2 = self.x, self.y, self.n_input, self.n_hidden_1, self.n_hidden_2
		weights, biases = tf_machine_wb(  x, y, n_input, n_hidden_1, n_hidden_2, n_classes = 1)
	...
	def multilayer_perceptron( self):
		...
		Development
		--------------
		Later, this function will be updated for further functionality. 
	...
	def tf_machine( self):
		...
		y, pred, learning_rate = self.y, self.pred, self.learning_rate
		cost = tf.reduce_mean( tf.square(y - pred))
		optimizer = tf.train.AdamOptimizer(learning_rate=learning_rate).minimize(cost) 
	...
	def run_opt( self, batch_xs, batch_ys):
		# sess.run(optimizer, feed_dict={x: batch_xs, y: batch_ys, keep_prob: keep_prob_train})
		sess, optimizer, x, y = self.sess, self.optimizer, self.x, self.y
		sess.run(optimizer, feed_dict={x: batch_xs, y: batch_ys})

	def run_cost( self, batch_xs, batch_ys):
		sess, cost, x, y = self.sess, self.cost, self.x, self.y
		return sess.run(cost, feed_dict={x: batch_xs, y: batch_ys})

	def run_pred( self, batch_xs_test):
		sess, pred, x = self.sess, self.pred, self.x 
		return sess.run( pred, feed_dict={x: batch_xs_test})
	...
	def fit(self, mnist, training_epochs = 100, display_step = 10, batch_size = 10, init_flag = True):
		...
		for epoch in range(training_epochs):
			...
			for i in range(total_batch):
				...
				# Fit training using batch data
				#sess.run(optimizer, feed_dict={x: batch_xs, y: batch_ys})
				self.run_opt( batch_xs, batch_ys)
				# Compute average loss
				# avg_cost += sess.run(cost, feed_dict={x: batch_xs, y: batch_ys})/total_batch
				avg_cost += self.run_cost( batch_xs, batch_ys)/total_batch
			# Display logs per epoch step
			if epoch % display_step == 0:
				print( "Epoch:", epoch)
				print( "Train-Epoch:", '%04d' % (epoch+1), "cost=", "{:.9f}".format(avg_cost))
				...
				# Fit training using batch data
				# sess.run(optimizer, feed_dict={x: batch_xs_test, y: batch_ys_test})
				avg_cost = self.run_cost( batch_xs_test, batch_ys_test)
				# Display logs per epoch step
				print( "Test:", "cost=", "{:.9f}".format(avg_cost))
				pred_test = self.run_pred( batch_xs_test)
				r2 = metrics.r2_score( batch_ys_test, pred_test)
				print( "R2 of test data:", r2)
...
class SDNN_dropout( DNN_dropout): # Smart DNN
	...
	def fit_1(self, mnist, training_epochs = 100, display_step = 10, batch_size = 10, init_flag = True, keep_prob_train = 1.0):
		"""
		fit() for new model which use a predicted output with the previously trained model.  
		"""
		sess = self.sess
...
class DNN_Layer_Dropout( DNN_Layer):
	...
	def multilayer_perceptron(self):
		x, keep_prob, weights, biases = self.x, self.keep_prob, self.weights, self.biases
		layer1, layer2, pred = open_multilayer_perceptron_dropout(x, keep_prob, weights, biases)
		self.layer1, self.layer2, self.pred = layer1, layer2, pred
	...
	def run_opt( self, batch_xs, batch_ys):
		# sess.run(optimizer, feed_dict={x: batch_xs, y: batch_ys, keep_prob: keep_prob_train})
		sess, optimizer, x, y, keep_prob = self.sess, self.optimizer, self.x, self.y, self.keep_prob
		keep_prob_train = self.keep_prob_train
		sess.run(optimizer, feed_dict={x: batch_xs, y: batch_ys, keep_prob: keep_prob_train})

	def run_cost( self, batch_xs, batch_ys):
		sess, cost, x, y, keep_prob = self.sess, self.cost, self.x, self.y, self.keep_prob
		return sess.run(cost, feed_dict={x: batch_xs, y: batch_ys, keep_prob: 1.0})

	def run_pred( self, batch_xs_test):
		sess, pred, x, keep_prob = self.sess, self.pred, self.x, self.keep_prob 
		return sess.run( pred, feed_dict={x: batch_xs_test, keep_prob: 1.0})

	def fit(self, mnist, training_epochs = 100, display_step = 10, batch_size = 10, init_flag = True, keep_prob_train = 1.0):
		self.keep_prob_train = keep_prob_train
		return super().fit( mnist=mnist, training_epochs=training_epochs, display_step=display_step, batch_size=batch_size, init_flag=init_flag)

class SDNN_Layer_Dropout( DNN_Layer_Dropout):
	def __init__( self, n_nodes_l, learning_rate = 0.01):
		super().__init__( n_nodes_l, learning_rate = learning_rate)	
	...
	def fit_1(self, mnist, training_epochs = 100, display_step = 10, batch_size = 10, init_flag = True, keep_prob_train = 1.0):
		...
		for epoch in range(training_epochs):
			...
			if epoch % display_step == 0:
				...
				# Fit training using batch data
				avg_cost_test = sess.run(cost_1, feed_dict={x_1: batch_xs_test_1, y: batch_ys_test, keep_prob: keep_prob_train})
				# Display logs per epoch step
				print( "Test:", "cost=", "{:.9f}".format(avg_cost_test))
				pred_test = sess.run( pred_1, feed_dict={x_1: batch_xs_test_1, keep_prob: 1.0})
				r2 = metrics.r2_score( batch_ys_test, pred_test)
				print( "R2 of test data:", r2)
